...
int main()
{
  ...
  address.sin_port = htons(8080);//htons convert Ip port number into host byte order(liitle endian order to netwrok byte order(Big endian order)
  address.sin_addr.s_addr = inet_addr("192.168.1.161");//This funnction converts the string provided into ip address
  //Connect --> it will connect the socket with file descriptor socketId with to the address specified by addr

  char  message[200] = "Hello from client side";
  char reply[1000] = {0};
